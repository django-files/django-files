...
function initUserPopover() {
    ...
    const tpl = document.getElementById('user-menu-tpl')
    if (!btn || !tpl) return
    ...
    btn.addEventListener('shown.bs.popover', () => {
        const tip = document.querySelector('.user-menu-popover .popover-body')
        if (!tip) return

        // Copy buttons — scoped ClipboardJS instance for dynamically-added nodes
        if (typeof ClipboardJS !== 'undefined') {
            clipInstance = new ClipboardJS('.user-menu-copy', {
                container: tip,
                text: (el) => el.dataset.copyText,
            })
            clipInstance.on('success', (e) => {
                const el = e.trigger
                const orig = el.innerHTML
                el.innerHTML = '<i class="fa-solid fa-check me-2"></i> Copied!'
                setTimeout(() => {
                    el.innerHTML = orig
                }, 1200)
            })
        ...
        // Log out
        tip.querySelector('.user-menu-logout')?.addEventListener(
            'click',
            () => {
