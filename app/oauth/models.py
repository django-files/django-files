...
class CustomUser(AbstractUser):
    ...
    remove_exif_geo = models.BooleanField(
        default=False, verbose_name="No EXIF Geo", help_text="Removes geo exif data from images on upload."
    )
    remove_exif = models.BooleanField(
        default=False, verbose_name="No EXIF", help_text="Removes exif data from images on upload."
    )
    ...
    default_upload_name_format = models.CharField(
        max_length=4, choices=UploadNameFormats.choices, default=UploadNameFormats.NAME
    )
    default_file_private = models.BooleanField(
        default=False,
        verbose_name="Default File Private",
        help_text="If enabled file default to private when not specified.",
    )
    default_file_password = models.BooleanField(
        default=False, verbose_name="Auto File Password", help_text="Generates file password on upload."
    )
    ...
    user_avatar_choice = models.CharField(
        max_length=2, choices=UserAvatarChoices.choices, default=UserAvatarChoices.STORAGE
    )
...
class UserInvites(models.Model):
    ...
    storage_quota = models.PositiveBigIntegerField(
        default=None, null=True, blank=True, help_text="Futureser's storage quota in bytes."
    )
