...
- tus resumable uploads (`TUS_ENABLED`, on by default — every image runs tusd, so opt out with `TUS_ENABLED=False` for a custom deployment that doesn't):
  - tusd owns transport at `/tus/`: a sidecar container in the compose stacks, a local supervisord program (loopback-only, `docker/supervisord.conf`) in the all-in-one image — `docker/docker-entrypoint.sh` aliases `tusd` to loopback in `/etc/hosts` so nginx's static `proxy_pass http://tusd:8080` resolves unmodified in both; nginx 404s `/api/tus/hook/` externally either way
  - hook endpoint `app/api/tus.py` (pre-create: auth + quota before bytes move; post-finish: enqueue import)
  - hook fails closed: 404 unless `TUS_ENABLED`, 403 without the shared secret (`TUS_HOOK_SECRET` env, else `/data/media/db/tus-hook.secret` generated by the nginx entrypoint; tusd passes it as `?secret=` in the hook URL)
  - `home.tasks.import_tus_upload` rechecks size/quota against the on-disk file, then imports via `process_file(LocalFile(...))`; `cleanup_tus_uploads` beat task sweeps `/data/media/tus`
